...
class StorageService {
  ...
  Future<List<StorageItem>> readListSecureData(
      List<String> keys, String widgetTitle) async {
    List<StorageItem> list = [];
    var readData;
    debugPrint("Reading List secured data");
    debugPrint(keys.toString());
    debugPrint(keys.length.toString());
    for (int i = 0; i < keys.length; i++) {
      if (keys[i] == 'Description') {
        readData = await _secureStorage.read(
            key: '$widgetTitle ${keys[i]}', aOptions: _getAndroidOptions());
      } else {
        readData = await _secureStorage.read(
            key: keys[i], aOptions: _getAndroidOptions());
      }

      if (readData != null) {
        list.add(StorageItem(keys[i], readData.toString()));
        debugPrint(keys[i]);
        debugPrint(readData);
      }
    }
